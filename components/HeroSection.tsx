...
export function HeroSection({ theme, featuredProduct }: HeroSectionProps) {
  ...
  const salePrice = featuredProduct
    ? getSalePrice(featuredProduct.priceRange.minVariantPrice.amount)
    : null;
      ...
      <div className="relative grid grid-cols-1 lg:grid-cols-2 gap-8 items-center px-10 py-14 lg:px-16">
        <div className="space-y-6 z-10">
          ...
          <p className="text-base leading-relaxed max-w-md" style={{ color: "var(--color-text-muted)" }}>
            {featuredProduct?.description?.slice(0, 120) || theme.tagline}
          </p>
        ...
        <div className="relative flex items-center justify-center">
          ...
          {salePrice && (
            ...
            >
              <p className="text-xs font-medium" style={{ color: "var(--color-text-muted)" }}>Precio</p>
              <p className="text-xl font-black" style={{ color: "var(--color-accent)" }}>{salePrice}</p>
            </div>
          )}
        </div>
      </div>
